...
class NewReleaseStrategy(BaseDiscoveryStrategy):
    """
    Crawls New Releases hubs.
    """
    @classmethod
    def get_metadata(cls) -> StrategyMetadata:
        return StrategyMetadata(
            id="strat_new_releases",
            name="New Releases",
            priority=70,
            ...
            notes="High search traffic, early discounts"
        )
        
    def generate_targets(self, provider: PluginManifestV2, budget_allocation: int) -> List[SearchTargetDTO]:
        entrypoint = provider.entrypoints.get("NEW_RELEASES")
        if not entrypoint or budget_allocation <= 0:
            return []
            
        targets = []
        trace_ctx = TraceContext(
            trace_id=generate_trace_id("nwr"),
            provider=provider.name,
            strategy="strat_new_releases"
        )
        ...
        for page in range(1, pages_to_crawl + 1):
            targets.append(SearchTargetDTO(
                ...
                expected_content="NEW_RELEASES_GRID",
