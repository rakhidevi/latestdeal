...
Route::get('/purge-duplicates', function() { $deals = \App\Models\Deal::orderBy('id')->get(); $titles = []; $deleted = 0; foreach ($deals as $deal) { if (isset($titles[$deal->title])) { $deal->delete(); $deleted++; } else { $titles[$deal->title] = true; } } return 'Deleted ' . $deleted . ' duplicate deals!'; });
