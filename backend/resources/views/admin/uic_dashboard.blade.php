        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
            ...
            <ul class="divide-y divide-gray-100 dark:divide-slate-700">
                @forelse($recentClicks ?? [] as $click)
                    <li class="px-6 py-4 flex justify-between items-center">
                        <div>
                            <p class="font-medium text-gray-900 dark:text-white text-sm line-clamp-1">{{ $click->deal->title ?? 'Direct Link' }}</p>
                            <p class="text-xs text-gray-500 mt-0.5">{{ $click->created_at ? $click->created_at->diffForHumans() : 'Recently' }} via {{ $click->deal->merchant->name ?? 'Amazon India' }}</p>
                        </div>
                    </li>
                @empty
                    <li class="px-6 py-4 text-gray-500 text-center">No affiliate conversions logged yet.</li>
                @endforelse
            </ul>
        </div>
    </div>
