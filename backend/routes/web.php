...
Route::get('/debug-deal-52', function() { return \App\Models\Deal::find(52)->getAttributes(); });
Route::get('/debug-db', function() { return \App\Models\Deal::where('title', 'LIKE', '%ZEEL%')->orderBy('id', 'desc')->first()->toJson(); });
Route::get('/cleanup-deals', function() { $deals = \App\Models\Deal::all(); $updated = 0; foreach ($deals as $deal) { $dirty = false; if (is_string($deal->trust_metrics)) { $decoded = json_decode($deal->trust_metrics, true); if (is_array($decoded)) { $deal->trust_metrics = $decoded; $dirty = true; } } if (is_string($deal->features)) { $decoded = json_decode($deal->features, true); if (is_array($decoded)) { $deal->features = $decoded; $dirty = true; } } if (is_string($deal->pros)) { $decoded = json_decode($deal->pros, true); if (is_array($decoded)) { $deal->pros = $decoded; $dirty = true; } } if (is_string($deal->cons)) { $decoded = json_decode($deal->cons, true); if (is_array($decoded)) { $deal->cons = $decoded; $dirty = true; } } if ($deal->editorial_summary) { $newSummary = preg_replace('/(?m)^.*?(?:Buy Now|Grab it here):\s*https?:\/\/[^\s]+.*$/iu', '', $deal->editorial_summary); $newSummary = trim($newSummary); if ($newSummary !== trim($deal->editorial_summary)) { $deal->editorial_summary = $newSummary; $dirty = true; } } if ($dirty) { $deal->save(); $updated++; } } return 'Cleaned up ' . $updated . ' deals!'; });
