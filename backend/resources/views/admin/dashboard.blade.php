...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700">
            ...
            <p class="text-3xl font-black text-gray-900 dark:text-white mt-2">{{ number_format($stats['total_visitors']) }}</p>
        </div>
        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700">
            <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Total Sessions</h3>
            <p class="text-3xl font-black text-gray-900 dark:text-white mt-2">{{ number_format($stats['total_sessions']) }}</p>
        </div>
        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700">
            <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Affiliate Clicks</h3>
            <p class="text-3xl font-black text-green-600 dark:text-green-400 mt-2">{{ number_format($stats['total_affiliate_clicks']) }}</p>
        </div>
        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700">
            <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Total Searches</h3>
            <p class="text-3xl font-black text-blue-600 dark:text-blue-400 mt-2">{{ number_format($stats['total_searches']) }}</p>
        </div>
    </div>
            ...
            <ul class="divide-y divide-gray-100 dark:divide-slate-700">
                @forelse($topSearches as $search)
                    <li class="px-6 py-4 flex justify-between items-center">
                        <span class="font-medium text-gray-700 dark:text-gray-300">{{ $search->search_query }}</span>
                ...
                @forelse($recentClicks as $click)
                    <li class="px-6 py-4 flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">{{ $click->created_at->diffForHumans() }} via {{ $click->deal->merchant->name ?? 'Unknown Merchant' }}</p>
                        </div>
                        <span class="text-xs text-gray-400 font-mono">{{ substr($click->visitor_uuid, 0, 8) }}...</span>
                    </li>
                @empty
